...
export default class CToken extends SmartContract {
  ...
  public static forNet(network: EthNet, symbol?: string): CToken {
    // Require a symbol to use CToken
    if (!symbol) {
      return null
    }
    const address = addresses[network][symbol];
    return new CToken(
      address,
      require(`../abis/${network}/compound/${symbol.toLowerCase()}.json`),
      decimals[symbol]);
  }
